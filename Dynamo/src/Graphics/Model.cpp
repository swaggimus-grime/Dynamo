...
Model::Model(std::shared_ptr<GPU> gpu, const std::string& path, std::shared_ptr<Shader> shader)
    :m_GPU(gpu), m_Shader(shader)
{
    ...
    for (UINT meshIndex = 0; meshIndex < scene->mNumMeshes; meshIndex++) {
        ...
        if (mesh->mMaterialIndex >= 0) {
            ...
            if (std::shared_ptr<Texture2D> diffuse = GetTexture(material, aiTextureType_DIFFUSE, 0))
                textures.push_back(diffuse);
            if (std::shared_ptr<Texture2D> specular = GetTexture(material, aiTextureType_SPECULAR, 1))
                textures.push_back(specular);
            if (std::shared_ptr<Texture2D> normal = GetTexture(material, aiTextureType_HEIGHT, 2))
                textures.push_back(normal);
        }

        std::shared_ptr<VertexBuffer<Vertex>> vbuff = std::make_shared<VertexBuffer<Vertex>>(m_GPU, vertices, shader->GetVSCode(), layout);
        std::shared_ptr<IndexBuffer> ibuff = std::make_shared<IndexBuffer>(m_GPU, indices);
        Mesh m = { vbuff, ibuff, textures };
...
void Model::Render() const
{
    ...
    for (const auto& m : m_Meshes) {
        ...
        m_GPU->GetDC()->DrawIndexed(m.IBuff->Size(), 0, 0);
    }
}

std::shared_ptr<Texture2D> Model::GetTexture(const aiMaterial* mat, aiTextureType type, UINT slot)
{
    aiString texName;
    ...
    return std::make_shared<Texture2D>(m_GPU, wbuff, slot);
}
