...
std::string BlendState::CreateHash(BLEND_MODE mode)
{
	return typeid(BlendState).name() + std::to_string(static_cast<UINT>(mode));
}

Shared<BlendState> BlendState::Evaluate(Graphics& g, BLEND_MODE mode)
{
	return Binds::Evaluate<BlendState>(g, mode);
}
