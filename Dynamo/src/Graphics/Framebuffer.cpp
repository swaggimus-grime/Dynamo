...
Framebuffer::Framebuffer(Graphics& g, UINT width, UINT height, BOOL depthEnable)
{
    ...
    g.SC().GetBuffer(0, __uuidof(ID3D11Resource), reinterpret_cast<void**>(&backBuff));
...
{
    g.DC().ClearRenderTargetView(m_ColorView.Get(), clearColor.m128_f32);
    g.DC().ClearDepthStencilView(m_DSView.Get(), D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.f, 0.f);
}

void Framebuffer::Bind(Graphics& g) const
{
