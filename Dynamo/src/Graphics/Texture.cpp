...
Texture2D::Texture2D(Graphics& g, const std::string& path, UINT slot)
	:m_Slot(slot), Bindable(CreateHash(path, slot))
{
	DirectX::ScratchImage image;
	DirectX::LoadFromWICFile(NarrowToWide(path), DirectX::WIC_FLAGS_IGNORE_SRGB, nullptr, image);
	if (!image.GetPixels())
...
Cubemap::Cubemap(Graphics& g, const std::string& texDir, UINT slot)
	:Bindable(CreateHash(texDir, slot)), m_Slot(slot)
{
	m_Slot = slot;
	...
	for (UINT i = 0; i < 6; i++) {
		...
		DirectX::ScratchImage image;
		DirectX::LoadFromWICFile(path.c_str(), DirectX::WIC_FLAGS_IGNORE_SRGB, nullptr, image);
